...
public final class ZipUtil {
     ...
     * 打包成zip
     *
     * @param fileList 文件或目录列表，如果是目录会递归打包
     * @param zipPath  zip包生成路径。完整路径名，包括文件名和扩展名
    ...
    private static void deleteFileIfExists(File zipFile) {
        if (zipFile.exists()) {
            zipFile.delete();
        }
    ...
    public static void main(String[] args) {
        ...
        String zipPath = "D:/myDemo.zip";

        File out = genZipFile(fileList, zipPath);
        System.out.println("处理成功:" + out);
    }
}